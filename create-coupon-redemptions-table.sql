-- Create Coupon Redemptions Table and Schema
-- This script creates the complete redemption tracking system from scratch

-- First, create the coupon_redemptions table
CREATE TABLE IF NOT EXISTS public.coupon_redemptions (
    ...
    -- Add unique constraint to prevent duplicate redemptions in same second
    CONSTRAINT coupon_redemptions_pkey PRIMARY KEY (coupon_id, user_id, redeemed_at)
);

-- Now add the enhanced columns for QR tracking
...
BEGIN
    ...
    IF TG_OP = 'DELETE' THEN
        UPDATE public.coupons 
        SET current_usage_count = GREATEST(0, COALESCE(current_usage_count, 0) - 1)
...
SELECT 
    ...
    COUNT(r.coupon_id) as total_redemptions,
...
GRANT ALL ON public.redemption_analytics TO authenticated;

-- Show table structure for verification
SELECT 'COUPON_REDEMPTIONS TABLE CREATED' as info;
SELECT column_name, data_type, is_nullable, column_default 
FROM information_schema.columns 
WHERE table_name = 'coupon_redemptions' AND table_schema = 'public'
ORDER BY ordinal_position;

-- Show coupons table usage columns
SELECT 'COUPONS TABLE USAGE COLUMNS' as info;
SELECT column_name, data_type, is_nullable, column_default 
FROM information_schema.columns 
...
BEGIN
    ...
    IF test_coupon_id IS NOT NULL AND test_user_id IS NOT NULL THEN
        -- Insert test redemption with unique timestamp to avoid duplicates
        INSERT INTO public.coupon_redemptions (
            ...
            'redemption_test_' || extract(epoch from now())::text || '_' || substr(md5(random()::text), 1, 8),
            test_coupon_id,
            test_user_id,
            '{"test": true, "redemption_id": "test_' || extract(epoch from now())::text || '"}',
            'used',
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Could not create test redemption: %', SQLERRM;
END $$;

-- Show sample redemption data
SELECT 'SAMPLE REDEMPTIONS (if any exist)' as info;
SELECT 
    r.id,
    r.status,
    r.redeemed_at,
...
FROM pg_policies
WHERE tablename = 'coupon_redemptions';

-- Final confirmation
SELECT 'SETUP COMPLETE' as info, 
       'The coupon_redemptions table has been created with all necessary columns, indexes, and RLS policies.' as message;